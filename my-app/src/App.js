...
function PunishTeam(teamID) {
	client.say(json.username[IdentityUserID], `Punishing the ${teamInfo[teamID].name}'s team`);
		for (let i = 0; i < teams[teamID].length; i++) {
			var duration = 60;
			client.say(json.username[IdentityUserID], `/timeout ${teams[teamID][i]} ${duration}`);
		}
		if (teams[teamID].length == 0) {
			client.say(json.username[IdentityUserID], `${teamInfo[teamID].name} has noone to punish! How sad....`);
        }
}

function WinnerTeam(teamID) {
	client.say(json.username[IdentityUserID], `The winner is ${teamInfo[teamID].name}`);
...
function ping() {
	client.say(json.username[IdentityUserID], `Chat is being pinged`);
}
...
function refreshLists() {
	...
	holder.textContent = "";
	for (let i = 0; i < teamInfo.length; i++) {
		var newText = document.createTextNode(teamInfo[i].name + " ");
		holder.appendChild(newText);
		var newButton = document.createElement("button");
		newButton.textContent = "Punish";
		...
		newButton2.textContent = "Winner";
		newButton2.onclick = function () {
			WinnerTeam(i);
		}
		holder.appendChild(newButton2);
	...
	return (
		<div className="App">
		  <header className="App-header">
			  <button onClick={ping}>Ping Chat!</button>
			  <button onClick={refreshLists}>Refresh</button>
			  <div id="ListHolder">
				<p>Click refresh to generate teams</p>
			  </div>
		</header>
    </div>
  );
